...
from kaiwu_env.conf import yaml_alloc
from kaiwu_env.conf import yaml_monitor
from kaiwu_env.conf import yaml_rainbow
...
import logging
import yaml
from kaiwu_env import yaml_arena
...
def reset_global_logger(log_dir, svr_name, log_file_prefix=None):
    logger = KaiwuLogger()
    log_file_prefix = yaml_logging.log_file_prefix if log_file_prefix == None else log_file_prefix
    if yaml_arena.proc_id == "default":
        logger.setLoggerFormat(f"{log_dir}/{svr_name}/{log_file_prefix}{datetime.datetime.now().strftime('%Y-%m-%d-%H')}.log", params=yaml_logging)
    else:
        logger.setLoggerFormat(f"{log_dir}/{svr_name}/{log_file_prefix}{yaml_arena.proc_id}.log", params=yaml_logging)
    return logger

def run_alloc_proxy_proc(file_path=None, section=None):
    file_path = os.path.join(kaiwu_env.__path__[0], yaml_alloc.file_path) if file_path == None else file_path
    section = yaml_alloc.section if section == None else section
    # CONFIG.parse_configure(['main_system'], '/data/projects/Skylarena/kaiwu_env/conf/configure_system.toml')
...
class GlobalMonitor:
    def __init__(self, logger, file_path=None, section=None):
        file_path = os.path.join(kaiwu_env.__path__[0], yaml_monitor.file_path) if file_path == None else file_path
...
class GlobalRainbow:
    def __init__(self, logger, file_path=None, section=None):
        ...
        file_path = os.path.join(kaiwu_env.__path__[0], yaml_rainbow.file_path) if file_path == None else file_path
