...
class VariableMgrIndependent(object):
    ...
    def savable_variables(self):
        ...
        for v in tf.global_variables():
            ...
            if (
                not v.name.startswith("input_datas")
    ...
    def trainable_variables_on_device(self, writable=False):
        del writable
        params = [v for v in tf.trainable_variables() if v.name.startswith("v0/")]
        return params
...
def get_loss_scale_update_op(loss_scale, loss_scale_normal_steps, inc_loss_scale_every_n, increase_loss_scale):
    ...
    def increase_loss_scale_func():
        new_loss_scale = tf.cond(
            tf.is_finite(loss_scale * increase_loss_scale),
            lambda: loss_scale * increase_loss_scale,
            lambda: loss_scale,
        )
        return tf.group(tf.assign(loss_scale_normal_steps, 0), tf.assign(loss_scale, new_loss_scale))
...
def append_gradients_with_loss_scale(training_ops, get_apply_gradients_ops_func, loss_scale_params, grad_has_inf_nan):
    ...
    if loss_scale is None or not enable_auto_loss_scale:
        ...
        def update_op_if_nan_or_inf():
            return tf.group(
                tf.assign(loss_scale, tf.maximum(1.0, loss_scale * decrease_loss_scale)),
                tf.assign(loss_scale_normal_steps, 0),
            )

        def update_op_if_no_nan_or_inf():
            return tf.group(
                get_loss_scale_update_op(
                    loss_scale,
                    loss_scale_normal_steps,
                    inc_loss_scale_every_n,
                    increase_loss_scale,
