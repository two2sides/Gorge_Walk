...
class MsgBuff:
    ...
    def update(self, recv_msg):
        ...
        if retry_num >= CONFIG.socket_retry_times:
            raise RuntimeError("gamecore %s failed to put msg into input queue" % (self.client_address))
        if self.exit_flag:
            raise RuntimeError("gamecore %s exit..." % (self.client_address))
        ...
        while retry_num < CONFIG.socket_retry_times and not self.exit_flag:
            ...
            retry_num += 1
        if retry_num >= CONFIG.socket_retry_times:
            raise RuntimeError("gamecore %s failed to get msg from output queue" % (self.client_address))
        if self.exit_flag:
            raise RuntimeError("gamecore %s exit..." % (self.client_address))
    ...
    def recv_msg(self):
        ...
        if json_str is None:
            raise RuntimeError("gamecore %s failed to get msg from input queue" % (self.client_address))
        return json_str
    ...
    def send_msg(self, json_str):
        ...
        if json_str is not None:
            raise RuntimeError("gamecore %s failed to put msg into output queue" % (self.client_address))
