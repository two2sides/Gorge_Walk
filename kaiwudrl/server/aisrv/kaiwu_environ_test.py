...
from kaiwudrl.interface.action import Action
from kaiwudrl.interface.exception import ClientQuitException, SkipEpisodeException
from kaiwudrl.interface.policy import Policy, PolicyBuilder
from kaiwudrl.interface.network import Network
from kaiwudrl.interface.reward import Reward
from kaiwudrl.interface.reward_shaper import RewardShaper
from kaiwudrl.interface.run_handler import RunHandler
from kaiwudrl.interface.state import State
from kaiwudrl.server.aisrv.flatbuffer.kaiwu_msg import *
...
from kaiwudrl.server.aisrv import kaiwu_environ
...
class MockRunHandler(RunHandler):
    ...
    def on_update_rsp(self, actions, extra_info=None):
        return {0: b"Hello World!", 1: b"Hello World!"}
...
class MockKaiwuMsg:
    ...
    def build_update_req():
        ...
        update_req = KaiwuMsgHelper.encode_update_req(
            builder, "fake client_id", 1, {0: [b"Hello World 0"], 1: [b"Hello World 1"]}
        )
...
class TestKaiwuEnviron(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.kaiwu_environ = KaiwuEnviron(
            simu_ctx=fake_simu_ctx,
            exit_flag=exit_flag,
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_init")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.next_valid")
    ...
    def test_reset_assert(self, mock_next_valid):
        ...
        fake_value_2 = {
            fake_agent_id: False,
            "_all_done_": False,
        }  # 为了跳出while loop将True改成False

        mock_next_valid.return_value = fake_value_0, fake_value_1, fake_value_2

        self.assertRaises(AssertionError, self.kaiwu_environ.reset)

    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_step")
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.next_valid")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.send_rsp")
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.finish")
    def test_reject(self, mock_finalize, mock_send_rsp):
        ...
        该函数会构造一个reject的消息, 发送给客户端, 最后调用finish函数做收尾工作
        """
    ...
    def test_finish(self):
        ...
        """
        self.kaiwu_environ.finish()

        self.kaiwu_environ.exit_flag.value = False  # reset

    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_ep_start")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_agent_start")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_update")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_agent_end")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_ep_end")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_event")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.handle_heartbeat")
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.recv_req")
    ...
    def test_handle_step(self):
        ...
        fake_actions = {0: MockAction(), 1: MockAction()}
        fake_extra_info = {0: {}, 1: {}}
    ...
    @mock.patch("kaiwudrl.server.aisrv.kaiwu_environ.KaiwuEnviron.send_rsp_to_client")
