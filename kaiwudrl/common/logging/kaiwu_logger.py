...
class KaiwuLogger(object):
    ...
    def setLoggerFormat(self, file_name, filter_content=None):
        ...
        self.logger = logger.bind(json="")
        self.logger = self.logger.patch(lambda r: r["extra"].update(json=self.json_formatter(r)))
    ...
    def debug(self, msg, *args, **kwargs):
        return self.logger.opt(depth=self.depth).debug(
            self.make_msg_content(msg, self.is_not_server(*args)), *args, **kwargs
        )
    ...
    def log(self, level, msg, *args, **kwargs):
        return self.logger.opt(depth=self.depth).log(
            level,
            self.make_msg_content(msg, self.is_not_server(*args)),
            *args,
            **kwargs,
        )
