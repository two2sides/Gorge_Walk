...
class TestClusterConf(unittest.TestCase):
    ...
    def test_cluster_spec(self):
        self.assertEqual(
            self.learner_cluster_conf.cluster_spec(),
            {
                "learner": ["localhost:8001"],
                "actor": ["localhost:8002", "localhost:8002"],
            },
        )
        self.assertEqual(
            self.actor_cluster_conf_0.cluster_spec(),
            {
                "learner": ["localhost:8001"],
                "actor": ["localhost:8002", "localhost:8002"],
            },
        ...
        self.assertEqual(
            self.actor_cluster_conf_3.cluster_spec(),
            {
                "learner": ["localhost:8001"],
                "actor": ["localhost:8002", "localhost:8002"],
            },
    ...
    def test_actor_cluster_conf(self):
        ...
        cluster_conf = ClusterConf(
            CONFIG.learner_ip_addrs.split(","),
            CONFIG.actor_ip_addrs.split(","),
