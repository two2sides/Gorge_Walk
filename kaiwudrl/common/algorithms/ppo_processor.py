...
from kaiwudrl.common.algorithms.expr_processor import RLExprProcessor
...
class PPOProcessor(RLExprProcessor):
    def __init__(self, simu_ctx, agent_ctx, policy_id):
        ...
        self.logger.setLoggerFormat(
            f"/{CONFIG.svr_name}/sample_log_{datetime.datetime.now().strftime('%Y-%m-%d-%H')}.log",
            "sample",
        )
    ...
    def _gen_expr(self):
        ...
        neg_logprobs = {
            f"old_neg_logp_{action_name}": pred_output[self._agent_ctx.agent_id][f"neg_logprob_{action_name}"]
    ...
    def _proc_exprs(self):
        ...
        self._exprs = [
            expr for expr in self._exprs if expr.step >= local_step or random.random() <= CONFIG.expr_skip_rate
        ]
        valid_frame_cnt = len(self._exprs)
        skip_frame_cnt = total_frame_cnt - valid_frame_cnt
        if valid_frame_cnt == 0:
            self.logger.warning(
                f"sample Got 0 valid frame, maybe all expr.step is lower than local_step({local_step}), "
        ...
        train_data.update({k: v[:-1] if not CONFIG.use_rnn else split_seq(v[:-1]) for k, v in a.items()})
        train_data.update({k: v[:-1] if not CONFIG.use_rnn else split_seq(v[:-1]) for k, v in neg_logp.items()})
        train_data.update(
            {
                ...
                "s": s[:-1].reshape(-1)
                if not CONFIG.use_rnn
                else np.amin(split_seq(s[:-1], pad_value=local_step), axis=1).reshape(-1, 1),
            }
        )
