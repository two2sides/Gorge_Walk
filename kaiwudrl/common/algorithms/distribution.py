...
class CategoricalDist(Pd):
    ...
    def selected_logprob(self, actions):
        selected_logprob = -1.0 * tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=actions)
    ...
    def kl_divergence(old_dist, new_dist):
        ...
        kl_sy = tf.reduce_sum(
            old_dist.action_probs * (old_dist.action_logprobs - new_dist.action_logprobs),
            axis=-1,
        )
    ...
    def sample(self, play_mode_tensor):
        ...
        if CONFIG.gaussian_dist_add_noise:
            return tf.cond(
                ...
                lambda: sample,
                lambda: sample
                + tf.distributions.Normal(loc=0.0, scale=CONFIG.gaussian_dist_noise_stddev).sample(
                    sample_shape=tf.shape(self.mean)
                ),
            )
        return sample
