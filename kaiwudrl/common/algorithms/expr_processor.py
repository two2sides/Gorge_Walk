...
class RLExprProcessor(ExprProcessor):
    ...
    def proc_exprs(self):
        ...
        self._reward_sum = sum(
            [
                expr.reward.get_reward() if not summed_rwd else 0
                for summed_rwd, expr in zip(self._summed_rewards, self._exprs)
            ]
        )
        self._in_reward_sum = sum(
            [
                expr.reward.get_in_reward() if not summed_rwd else 0
                for summed_rwd, expr in zip(self._summed_rewards, self._exprs)
            ]
        )
        self._ex_reward_sum = sum(
            [
                expr.reward.get_ex_reward() if not summed_rwd else 0
                for summed_rwd, expr in zip(self._summed_rewards, self._exprs)
            ]
        )
