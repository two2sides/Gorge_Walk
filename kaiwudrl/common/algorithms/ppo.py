...
from kaiwudrl.common.algorithms.model import ModelSpec
from kaiwudrl.common.config.config_control import CONFIG
from kaiwudrl.common.algorithms.ckpt_saver_hook import (
    CheckpointSaverHook,
    CkptSaverListener,
)
...
class PPO(Model):
    ...
    def build_model(self, mode, input_tensors):
        # 使用绝对偏移计算decay
        relative_step = False
        self.step = self.current_step(CONFIG.restore_dir + f"/{CONFIG.app}_{CONFIG.algo}", relative_step)

        self.state_tensors = {state_name: input_tensors[state_name] for state_name in self.network.state_space}
    ...
    def ckpt_saver_hook(self, var_list=None):
        ...
        listener = CkptSaverListener(
            inputs={**self.state_tensors, "play_mode": self.play_mode},
            outputs={
                **self.sample_action,
                **self.sample_neg_logprob_action,
                "v": self.logits_v,
    ...
    def _build_policy(self, input_tensors, mode):
        ...
        with tf.compat.v1.variable_scope("sample_action"):
            ...
            self.dist = {
                action_name: self.network.action_space[action_name].pdclass(logits_p)
                for action_name, logits_p in self.logits_p.items()
            }

            self.sample_action = {action_name: dist.sample(self.play_mode) for action_name, dist in self.dist.items()}
            self.sample_neg_logprob_action = {
    ...
    def _build_loss(self, input_tensors):
        ...
        with tf.compat.v1.variable_scope("loss"):
            ...
            self.action_dict = {
                action_name: tf.reshape(input_tensors[action_name], [-1]) for action_name in self.network.action_space
            }

            self.old_neg_logp_dict = {
                action_name: tf.reshape(input_tensors[f"old_neg_logp_{action_name}"], [-1])
            ...
            self.clip_range = tf.compat.v1.train.exponential_decay(
                float(CONFIG.ppo_clip_range),
                self.step,
            ...
            self.vpred_clipfrac = tf.reduce_mean(
                tf.cast(
                    tf.greater(tf.abs(logits_v - self.old_vpred), self.clip_range),
                    tf.float32,
                )
            )
            vf_losses1 = tf.square(logits_v - self.y_r)  # Unclipped value
            vf_losses2 = tf.square(vpredclipped - self.y_r)  # Clipped value
            self.vf_loss = 0.5 * tf.reduce_mean(tf.maximum(vf_losses1, vf_losses2) * self.m)
            ...
            for action_name, dist in self.dist.items():
                ...
                pg_loss = tf.reduce_mean(tf.maximum(pg_losses, pg_losses2) * self.m)
                self.policy_loss[action_name] = pg_loss

                # approxkl
                approxkl = 0.5 * tf.reduce_mean(tf.square(neg_logp_action - old_neg_logp_action) * self.m)
    ...
    def _build_optimizer(self):
        with tf.compat.v1.variable_scope("optimizer"):
            ...
            self.opt = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr, epsilon=float(CONFIG.ppo_epsilon))
