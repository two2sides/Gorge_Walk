...
# @file reverb_replay_buffer_benchmark.py
# @brief
# @author kaiwu
# @date 2023-11-28
...
from kaiwudrl.common.replay_buffer.replay_buffer_benchmark_common import BenchmarkData
from kaiwudrl.common.replay_buffer.reverb_replay_buffer import ReverbReplayBuffer
...
def send_data_proc():
    ...
    with client.writer(max_sequence_length=args.max_sequence_length, chunk_length=args.chunk_length) as writer:
        bgn_time = time.time()
        cnt = 0
        while True:
...
def build_limiter(name, batch_size):
    ...
    if name == "MinSize":
        ...
        error_buffer = 1000
        rate_limiter = reverb.rate_limiters.SampleToInsertRatio(samples_per_insert, batch_size, error_buffer)
    else:
        rate_limiter = None
    return rate_limiter
...
def make_dataset(table_names):
    ...
    dataset = (
        tf.data.Dataset.from_tensor_slices(table_names)
        .interleave(
