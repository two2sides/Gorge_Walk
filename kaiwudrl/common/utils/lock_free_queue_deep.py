...
class SafeQueue:
    ...
    As such, no guarantee can be made about the order in while objects are put in the queue
    during threaded access.
...
class SharedCircBuf:
    """A circular buffer for numpy arrays that uses shared memory for inter-process
    communication."""
    ...
    def __init__(self, queue_size, ary_template, keys):
        ...
        for i in range(queue_size):
            ...
            for skey in range(len(keys)):
                ...
                data.append(np.frombuffer(sarray, dtype=elem_dtype, count=elem_size).reshape(elem_shape))
    ...
    class Guard:
        ...
        def __init__(self, out_queue, arys, idx_op):
            """
            The guard object returns ary, and once the guard ends the value of
            idx is put into queue.
            Used to put an element index representing a token back into the
            buffer queues once operations on the element are complete.
        ...
        def __exit__(self, *args):
            # put the location of written array in read queue
