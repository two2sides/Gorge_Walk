...
class PrometheusUtils(object):
    def __init__(self, logger) -> None:
        ...
        # 默认的labels, ip、task_id, app属性
        self.default_labels = ["ip", "task_id", "app"]
    ...
    def counter_use(self, server_name, item_name, item_help, value, pid=None):
        ...
        if regist_name not in self.c_maps:
            self.c_maps[regist_name] = Counter(
                item_name,
                item_help,
        ...
        self.c_maps[regist_name].labels(self.host, self.task_id, CONFIG.app).inc(value)
    ...
    def histogram_use(self, server_name, item_name, item_help, value, pid=None):
        ...
        if regist_name not in self.h_maps:
            self.h_maps[regist_name] = Histogram(
                item_name,
                item_help,
                registry=self.registry,
                labelnames=self.default_labels,
        ...
        self.h_maps[regist_name].labels(self.host, self.task_id, CONFIG.app).observe(value)
    ...
    def summary_use(self, server_name, item_name, item_help, value, pid=None):
        ...
        if regist_name not in self.s_maps:
            self.s_maps[regist_name] = Summary(
                item_name,
                item_help,
                registry=self.registry,
                labelnames=self.default_labels,
        ...
        self.s_maps[regist_name].labels(self.host, self.task_id, CONFIG.app).observe(value)
    ...
    def gauge_use(self, server_name, item_name, item_help, item_value, pid=None):
        ...
        self.g_maps[regist_name].labels(self.host, self.task_id, CONFIG.app).set(item_value)
