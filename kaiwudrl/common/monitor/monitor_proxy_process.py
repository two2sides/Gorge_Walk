...
class MonitorProxy(multiprocessing.Process):
    ...
    def before_run(self):
        ...
        self.logger.setLoggerFormat(
            f"/{CONFIG.svr_name}/monitor_proxy_pid{pid}_log_{datetime.datetime.now().strftime('%Y-%m-%d-%H')}.log",
            CONFIG.svr_name,
        )
        ...
        self.logger.info(f"monitor_proxy process start success at pid is {pid}")
    ...
    def send_to_prometheus(self, monitor_data):
        ...
        for pid, data in monitor_data.items():
            for monitor_name, montor_value in data.items():
                if isinstance(montor_value, list):
                    ...
                    self.prometheus_utils.gauge_use(CONFIG.svr_name, monitor_name, monitor_name, montor_value, pid)
